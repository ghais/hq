...
class StrikeSpace k where
  strikeSpaceToCash :: Surface v k -> k -> YearFrac -> Spot -> Forward -> Vol -> VolShift -> Strike
  cashToStrikeSpace :: Surface v k -> Strike -> YearFrac -> Spot -> Forward -> Vol -> VolShift -> k

instance StrikeSpace Strike where
  strikeSpaceToCash _ x _ _ _ _ _ = x
  cashToStrikeSpace _ k _ _ _ _ _ = k


instance StrikeSpace AbsRelStrike where
  strikeSpaceToCash _ (AbsRel x) _ _ (Forward f) _ _ = Strike $ x + f
  cashToStrikeSpace _ (Strike k) _ _ (Forward f) _ _ = AbsRel $ k - f

instance StrikeSpace LogRelStrike where
  strikeSpaceToCash _ (LogRel x) _ _ (Forward f) _ _ = Strike $ f * exp x
  cashToStrikeSpace _ (Strike k) _ _ (Forward f) _ _ = LogRel $ log $ k - f

instance StrikeSpace MoneynessForwardStrike  where
  strikeSpaceToCash _ (MoneynessForward x) (YearFrac t) _ (Forward f) (Vol atmVol) _ =
    Strike $ x * sqrt t * atmVol + f

  cashToStrikeSpace _ (Strike k) (YearFrac t) _ (Forward f) (Vol atmVol) _ =
    MoneynessForward $ (k - f) / (atmVol * sqrt t)

instance StrikeSpace LogMoneynessForwardStrike  where
  strikeSpaceToCash _ (LogMoneynessForward x) (YearFrac t) _ (Forward f) (Vol atmVol) (VolShift slnShift) =
    Strike $ (f + slnShift) * exp (x * (sqrt t) * atmVol) - slnShift

  cashToStrikeSpace _ (Strike k) (YearFrac t)  _ (Forward f) (Vol atmVol) (VolShift slnShift) =
    LogMoneynessForward $ (log ((k - slnShift) / (f + slnShift))) / (atmVol * sqrt t)

instance StrikeSpace LogMoneynessSpotStrike  where
  strikeSpaceToCash _ (LogMoneynessSpot x) (YearFrac t) (Spot s) _ (Vol atmVol) (VolShift slnShift) =
    Strike $ (s + slnShift) * exp (x * (sqrt t) * atmVol) - slnShift

  cashToStrikeSpace _ (Strike k) (YearFrac t)  (Spot s) _ (Vol atmVol) (VolShift slnShift) =
    LogMoneynessSpot $ (log ((k - slnShift) / (s + slnShift))) / (atmVol * sqrt t)

instance StrikeSpace MoneynessSpotStrike  where
  strikeSpaceToCash _ (MoneynessSpot x) (YearFrac t) (Spot s) _ (Vol atmVol) _ =
    Strike $ x * (sqrt t) * atmVol + s

  cashToStrikeSpace _ (Strike k) (YearFrac t) (Spot s) _ (Vol atmVol) _ =
...
extrapolateTotalVarFrom t0 surface@Surface{..} k t = let
  ...
  x           = cashToStrikeSpace surface k t spot f atmVol (slnShift surface)
  k'          = strikeSpaceToCash surface x t0 spot f0 atmVol0 (slnShift surface)
  x'          = cashToStrikeSpace surface k' t0 spot f0 atmVol (slnShift surface)
  in totalVar (surfaceVols M.! t0) x'


timeInterpolate :: (StrikeSpace k, TimeSlice v k) => TimeInterpolation -> Surface v k -> Strike -> YearFrac -> TotalVar
timeInterpolate Gatheral surface@Surface{..} k11 t =
  let (t1, smile1) = fromJust $ M.lookupLE t surfaceVols
      ...
      k1      = k11 $*$ (f1 / f)
      k2      = k11 $*$ (f2 / f)
      x1      = cashToStrikeSpace surface k1 t1 surfaceSpot (Forward f1) atmVol (slnShift surface)
      x2      = cashToStrikeSpace surface k2 t2 surfaceSpot (Forward f2) atmVol (slnShift surface)
...
timeInterpolate LinearInVol surface@Surface{..} k t =
  let f = surfaceForwardCurve t
      ...
      x      = cashToStrikeSpace surface k t surfaceSpot f atmVol (slnShift surface)
